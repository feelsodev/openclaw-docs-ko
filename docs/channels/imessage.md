...
상태: 레거시 외부 CLI 통합. 게이트웨이는 `imsg rpc`를 생성하고 stdio에서 JSON-RPC를 통해 통신합니다(별도의 데몬/포트 없음).
  ...
  - [**BlueBubbles (recommended)**](/channels/bluebubbles) - 새로운 설정을 위한 기본 iMessage 경로입니다.

  - [**Pairing**](/channels/pairing) - iMessage DM은 기본적으로 페어링 모드로 설정됩니다.

  - [**Configuration reference**](/gateway/configuration-reference#imessage) - 전체 iMessage 필드 참조.
...
::: details Dedicated bot macOS user (separate iMessage identity)
...
::: details Remote Mac over Tailscale (example)
  일반적인 토폴로지:
...
::: details imsg not found or RPC unsupported
...
::: details Group messages are ignored
  확인:
...
::: details Remote attachments fail
  확인:
...
::: details macOS permission prompts were missed
