...
OpenClaw는 `~/.openclaw/openclaw.json`에서 선택적 **JSON5** (주석과 후행 쉼표를 지원) 구성을 읽습니다.
...
::: details Set up a channel (WhatsApp, Telegram, Discord, etc.)
...
::: details Choose and configure models
...
::: details Control who can message the bot
...
::: details Set up group chat mention gating
...
::: details Configure sessions and resets
...
::: details Set up heartbeat (periodic check-ins)
```json5
{
  agents: {
    defaults: {
...
::: details Configure multi-agent routing
...
::: details Split config into multiple files ($include)
...
::: details config.apply (full replace)
...
::: details config.patch (partial update)
...
::: details Shell env import (optional)
...
::: details Env var substitution in config values
