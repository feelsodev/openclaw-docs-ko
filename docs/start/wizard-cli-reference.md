...
::: details Anthropic API key (recommended)
...
::: details Anthropic token (setup-token paste)
  아무 머신에서나 `claude setup-token`를 실행한 다음 토큰을 붙여넣습니다.
  이름을 지정할 수 있습니다. 공백은 기본값을 사용합니다.
:::


::: details OpenAI Code subscription (Codex CLI reuse)
  `~/.codex/auth.json`이 존재하는 경우 마법사는 이를 재사용할 수 있습니다.
:::


::: details OpenAI Code subscription (OAuth)
...
::: details Moonshot and Kimi Coding
