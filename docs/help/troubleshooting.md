...
::: details Dashboard or Control UI will not connect
```bash
openclaw status
openclaw gateway status
openclaw logs --follow
...
::: details Gateway will not start or service installed but not running
```bash
openclaw status
openclaw gateway status
openclaw logs --follow
...
::: details Channel connects but messages do not flow
```bash
openclaw status
openclaw gateway status
openclaw logs --follow
...
::: details Cron or heartbeat did not fire or did not deliver
```bash
openclaw status
openclaw gateway status
...
::: details Node is paired but tool fails camera canvas screen exec
```bash
openclaw status
openclaw gateway status
